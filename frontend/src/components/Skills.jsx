...
/* ---------- Sub-components (kept small to keep complexity low) ---------- */
...
const SunCore = () => (
  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
    ...
    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 text-center whitespace-nowrap">
      <div className="font-mono text-[10px] tracking-[0.4em] uppercase text-amber-200/80">Core</div>
      <div className="font-display text-base text-white mt-1">AI / ML Systems</div>
    </div>
  </div>
);

const SkillChip = ({ skill, isLit, isHover, color, onHover, onLeave, onClick }) => (
  <button
...
const OrbitSkills = ({ orbit, hovered, animationsActive, onHover, onLeave, onClick, isHighlighted }) => {
  const N = orbit.skills.length;
  const playState = animationsActive && !hovered ? "running" : "paused";
  ...
  return (
    ...
    >
      {orbit.skills.map((skill, i) => {
        const angle = (i / N) * 360;
        const isLit = isHighlighted("skill", skill.name);
        const isHover = hovered && hovered.name === skill.name;
        return (
          <div
            key={skill.name}
            className="absolute"
          ...
          >
            <div
              ...
              style={{
                animation: `spin-slow ${orbit.speed}s linear infinite reverse`,
                animationPlayState: playState
              }}
            >
              <SkillChip
                ...
                isHover={isHover}
                color={orbit.color}
                onHover={() => onHover({ orbit: orbit.name, name: skill.name })}
                onLeave={onLeave}
...
const MobileSkillsList = ({ orbits, isHighlighted, onClick }) => (
...
const Skills = () => {
  ...
  useEffect(() => {
    const onResize = () => {
      ...
      const fit = Math.min(window.innerWidth / needed, containerH / needed, 1.05);
  ...
  const headingStatus = highlight && highlight.source === "skill"
  ...
  return (
    <section id="sun" ref={wrapRef} className="relative min-h-screen py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-10 relative z-10">
        ...
        <MobileSkillsList orbits={skillsOrbits} isHighlighted={isHighlighted} onClick={handleSkillClick} />

        {/* Desktop 3-orbit solar system */}
        <div className="hidden lg:flex relative items-center justify-center" style={{ height: "min(80vh, 900px)" }}>
          <div
            className="relative"
